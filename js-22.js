...
function exampleOne(movies, minRating) {
  ...
  /* 초기화 함수 방식으로 상당히 간결하게 변수를 관리
  init()함수는 그자체로 객체이므로 반환값을 바로 변수에 할당할 수 있다. */

  /* 필터링된 영화, 정렬된 영화 및 상위 영화 변수 관리
  빈 배열을 초기화(initialize)했기 때문에 개발자들 사이에서도 이 자리에 무슨 데이터가 들어갈지 예상하기 쉽다.
  ...
  // 상위 영화 랭킹 로직 함수 최소 등급 이상의 영화 중 상위 3개 반환 | minRating 가 최소등급
  function getTopMovies(movies, minRating) {
    ...
    for (let movie of movies) {
      if (movie.rating >= minRating) {
        filteredMovies.push(movie); //filteredMovies는 최소 등급 이상인 것을 배열 한 값
      }
    }
    for (let i = 0; i < filteredMovies.length; i++) {
      sortedMovies.push(filteredMovies.sort(compareRatings)[i]); //sortedMovies는 filteredMovies를 내림차순으로 정렬 한 값
    }
    ...
    for (let i = 0; i < sortedMovies.length; i++) {
      if (i < 3) {
        //3등까지만 반환
        topMovies.push(sortedMovies[i]); //topMovies는 filteredMovies에서 3등을 가리는 것
      }
    }
    return topMovies;
  }
...
console.log(exampleOne(movies, 8)); //등급이 8이상인 것만 보여주게됨.
